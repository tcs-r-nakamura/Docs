...
│            │                      │ [請求書 v]     │
│ Upload中 1 │ ※ 選択時:            │ AI Suggest:    │
│            │  📄 invoice.pdf      │  請求書を検出  │
│ ─────      │  サイズ / ページ数   │  (将来要件)    │
│ AI Suggest │  Cost ¥12 (将来)     │ 校正設定       │
│  請求書を  │  [ファイルを変更]    │ [なし v]       │
│  検出      │                      │ 言語           │
│  Template= │ ※ 実行中:            │ ☑ 日本語       │
│  請求書    │  Upload [▓▓▓░░] 60% │ 前処理         │
...
AI による自動判定を全面に出した最新 AI-OCR 寄り。詳細設定は折り畳みで隠し、
AI Suggestion を中心に配置。詳細展開時は現実装の責務を網羅。
...
│                                            │
│ ※ 実行中:                                   │
│  ProgressBar + ステータス + 経過時間        │
│  [キャンセル]                               │
│                                            │
│ ※ 成功時:                                   │
│  ✓ 完了 + 処理時間 + [結果を表示]          │
├────────────────────────────────────────────┤
│ AI Suggestion (ファイル投入後の自動判定):    │
│  ・請求書を検出 (Template: 請求書)         │
│  ・Language: 日本語                        │
│  ・推奨 Engine: Azure DI                   │
│  ・Cost 推定: ¥12                          │
...
│ ▼ 展開時:                                   │
│   プロバイダー / エンジン / テンプレート    │
│   校正設定 (IsProofreadingAvailable 時)    │
│   言語 / 前処理 (CheckBox 群)              │
│   ⚠ 重複警告 (IsDuplicate)                 │
└────────────────────────────────────────────┘
[エラーメッセージ]         [キャンセル] [OCR実行]
...
凡例:
  ファイル領域: パターン① と同じ状態切替表示 (未選択/選択/実行中/成功)

  AI Suggestion セクション (将来要件):
    ファイル投入後に AI が帳票を分類し、推奨設定を提示
    [この提案で実行] = AI 推奨設定で即座に OCR 実行
    [手動で調整] = 詳細設定を開く

  詳細設定 (折り畳み、Expander):
    AI Suggestion で隠れているが、手動調整時に展開
    展開時は現実装の責務をすべて含む:
      Provider / Engine / Template / 校正設定 / 言語 / 前処理 / 重複警告
...
| 推奨 Engine 提示 | AI による最適 Provider/Engine 選定 |
...
| 高速運用 | ◎ |
| 初心者向け | ◎ |
| 責務カバー網羅 | ◎ (AI Suggestion + 詳細設定で全責務を含む) |

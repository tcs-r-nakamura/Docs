...
> **実装方針**: `ResultView.xaml` 内に `DateFilterDialog` として埋め込む。
...
>
> **配置**: `QuickDictionaryPanel`（ルート Grid 最上層・右寄せ）とは異なり、`DateFilterDialog` は左ペインの `Grid` 内に `Grid.RowSpan="3"` で重ねて配置する。ボタンがヘッダーにあるため、座標指定なしにボタン付近へ自然に表示できる。
...
| ファイル | 変更内容 |
|---------|--------|
| `client/OcrViewer/Presentation/ViewModels/ResultViewModel.cs` | `DateFrom`, `DateTo`, `IsDateFilterActive`, `IsDateFilterVisible`（フィルターダイアログ表示制御）, `SwapMessage`, `IsFilteredEmpty` プロパティ追加、`FilteredJobHistory` にフィルタロジック追加、`OpenDateFilterCommand` / `ClearDateFilterCommand` / `CloseDateFilterCommand` 追加、`_searchQuery` / `_statusFilter` に `[NotifyPropertyChangedFor(nameof(IsFilteredEmpty))]` 追加 |
...
```csharp
// SearchQuery / StatusFilter の変化時にも IsFilteredEmpty を更新する
[ObservableProperty]

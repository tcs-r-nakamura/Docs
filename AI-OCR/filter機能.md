...
> `IsVisible`（bool）を ViewModel に持ち、XAML の `Visibility` バインディングで表示／非表示を切り替える。
...
| 閉じるボタン | Button | `CloseDateFilterCommand` | - | ダイアログを閉じる（`IsVisible = false`、フィルタ設定は維持） |
...
| ファイル | 変更内容 |
|---------|--------|
| `client/OcrViewer/Presentation/ViewModels/ResultViewModel.cs` | `DateFrom`, `DateTo`, `IsDateFilterActive`, `IsVisible`（フィルターダイアログ表示制御）, `SwapMessage` プロパティ追加、`FilteredJobHistory` にフィルタロジック追加、`OpenDateFilterCommand` / `ClearDateFilterCommand` / `CloseDateFilterCommand` 追加 |

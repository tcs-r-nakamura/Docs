...
フィルタは （LINQ / in-memory）で即時反映される。

### 2-3. 日付項目の現状

| フィールド | APIレスポンス () | クライアントモデル () |
|-----------|-------------------------------------|--------------------------------------|
| | あり | あり（表示・ソートで使用中） |
| | あり | なし |
| | あり | **なし（今回追加対象）** |

**「処理日（OCR処理完了日時）」= はクライアントモデルに存在しない。**
...
| From | DatePicker | | null（未選択） | フィルタ開始日 |
| To | DatePicker | | null（未選択） | フィルタ終了日 |
...
日付フィルタが有効（From または To が入力済み）の場合、 のジョブは一覧から除外される。
...
| | プロパティ追加 |
| | に 追加、/ にマッピング追加 |
| ジョブ結果受信箇所（/ 呼び出し元） | を からセット |
| | プロパティ（または ）を公開 |
| | , プロパティ追加、 にフィルタロジック追加 |
| | From / To DatePicker UI 追加 |
...
| 既存テキスト検索 | 変更なし |
| 既存ステータスフィルタ | 変更なし |
| レコード型 | nullable プロパティ追加のみ（既存参照に影響なし） |
| ローカル保存済みの | フィールドがない既存データは として読み込まれる（後方互換） |
| API 通信 | は既に存在するため変更なし |
